...
final class AnnotationMappers {
    ...
    static void decorateOperationFromAnnotation(Operation annotation, io.swagger.v3.oas.models.Operation operation) {
        System.out.println("inside Annotation decorateOperationFromAnnotation");
        ...
        apiResponses.putAll(
                Arrays.stream(annotation.responses()).map(response -> {
                    System.out.println("response inside annotation" + response);
                    ApiResponse apiResponse = new ApiResponse();
                    apiResponse.description(response.description());
                    System.out.println("response.description()" + response.description());
                    System.out.println("response.content()" + response.content().length);
                    System.out.println("response.responseCode()" + response.responseCode());
                    if (response.content().length > 0) {
                        Arrays.stream(response.content()).forEach(content -> {
                            //System.out.println("response.content()" + content);
                            Content c = getContent(content);
                            System.out.println("response.examples()" + content.examples());
                            ExampleObject[] exampleObjects = new ExampleObject[2];
                            int i = 0;
                            for (ExampleObject exampleObject : content.examples()) {
                                Optional<Example> exampleOptional = AnnotationsUtils.getExample(exampleObject);
                                System.out.println("exampleOptional" + exampleOptional.get());
                                //exampleObjects[i] = example;
                                Map<String, Example> nesw = new HashMap<>();
                                nesw.put("i" + i, exampleOptional.get());
                                c.get(content.mediaType()).setExamples(nesw);
                                i++;
                            }
                            System.out.println("after set examp" + c);
                            System.out.println("response.content()" + content.array().schema().implementation());
                            if (!Void.class.equals(content.array().schema().implementation())) {
                                System.out.println("response.content()1111" + content.schema().example());
                                ...
                                System.out.println("response.content()2222" + content.schema().example());
